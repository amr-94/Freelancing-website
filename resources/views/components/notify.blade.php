        ...
        <button role="menuitem"
            ...
            @foreach (Auth::user()->unreadnotifications as $notify)
                <div class="flex flex-col gap-1">
                    <a href="{{ route('user_profile.index', $notify->data['message']['from_user_id']) }}"
                        class="block font-sans text-sm antialiased font-semibold leading-normal text-gray-700">
                        user {{ $notify->data['sender'] }} send message to you
                    </a>

                    <a href="{{ route('notify.read', $notify->id) }}"
                        class="flex items-center gap-1 font-sans text-sm antialiased font-medium text-blue-gray-500">
                        title : {{ $notify->data['message']['title'] }}
                    </a>
                    ...
                    <p class="flex items-center gap-1 font-sans text-sm antialiased font-medium text-blue-gray-500">
                        ...
                        from {{ $notify->created_at->diffForhumans() }}
                    </p>
                </div>
            @endforeach
